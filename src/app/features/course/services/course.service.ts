...
export class CourseService {
  ...
  //Exponer los cursos como una Signal de solo lectura para que los componentes puedan subscribirle a cambios
  ...
  enrollStudent(courseId: number): void {
    this.coursesData.update(currentCourses => 
      currentCourses.map(course => 
        //Incrementa enrolledStudents si el ID conicide y hay espacio
        course.id === courseId && course.enrolledStudents < course.maxStudents
        ? { ...course, enrollStudent: course.enrolledStudents + 1 }
        : course
      )
    );
